...
{
    var TypeNotSupportedErr: Label 'Type %1 is not supported.', Comment = 'Line or Document type';
    ...
    procedure GetAccountTableId(DocumentTableId: Integer): Integer begin
        case DocumentTableId of Database::"Sales Header", Database::"Sales Invoice Header", Database::"Sales Cr.Memo Header", Database::"Sales Shipment Header", Database::"Return Receipt Header": exit(Database::Customer);
        Database::"Purchase Header", Database::"Purch. Inv. Header", Database::"Purch. Cr. Memo Hdr.", Database::"Purch. Rcpt. Header", Database::"Return Shipment Header": exit(Database::Vendor);
        end;
    end;
    ...
    procedure GetNextEmailAddressFromCustomReportSelection(var CustomReportSelection: Record "Custom Report Selection"; DefaultEmailAddress: Text; ReportUsage: Enum "Report Selection Usage"; SequenceText: Text): Text var
    ...
    begin
        if AttachmentFileName = '' then if PostedDocNo = '' then begin
                if ReportUsage = ReportSelections.Usage::"P.Order".AsInteger()then AttachmentFileName:=StrSubstNo(PdfFileNamePluralMsg, EmailDocumentName)
                else
                    AttachmentFileName:=StrSubstNo(ReportAsPdfFileNamePluralMsg, EmailDocumentName);
            end
            else
                AttachmentFileName:=StrSubstNo(ReportAsPdfFileNameMsg, EmailDocumentName, PostedDocNo)end;
    procedure GetEmailSubject(PostedDocNo: Code[20]; EmailDocumentName: Text[250]; ReportUsage: Integer)Subject: Text[250]var
    ...
    begin
        ...
        if PostedDocNo = '' then Subject:=CopyStr(StrSubstNo(EmailSubjectPluralCapTxt, CompanyInformation.Name, EmailDocumentName), 1, MaxStrLen(Subject))
        else
            Subject:=CopyStr(StrSubstNo(EmailSubjectCapTxt, CompanyInformation.Name, EmailDocumentName, PostedDocNo), 1, MaxStrLen(Subject))end;
    ...
    begin
        ...
        case OptionNumber of 0: exit;
        1: begin
            SalesLine.SetRange("Document Type", SalesHeader."Document Type");
            SalesLine.SetRange("Document No.", SalesHeader."No.");
        end;
        2: SalesLine.Copy(SelectedSalesLine);
        end;
    ...
    local procedure SelectVendor(var Vendor: Record Vendor; SalesHeader: Record "Sales Header"): Boolean var
        VendorNo: Code[20];
    begin
    ...
    begin
        PurchaseHeader.Init();
        if SalesHeader."Document Type" in[SalesHeader."Document Type"::Invoice, SalesHeader."Document Type"::Order]then PurchaseHeader.Validate("Document Type", PurchaseHeader."Document Type"::Invoice)
        else
    ...
    begin
        PurchaseLineNo:=0;
        if SalesLine.Find('-')then repeat Clear(PurchaseLine);
                PurchaseLine.Init();
                PurchaseLine."Document No.":=PurchaseHeader."No.";
                PurchaseLine."Document Type":=PurchaseHeader."Document Type";
                PurchaseLineNo:=PurchaseLineNo + 10000;
                PurchaseLine."Line No.":=PurchaseLineNo;
                case SalesLine.Type of SalesLine.Type::" ": PurchaseLine.Type:=PurchaseLine.Type::" ";
                SalesLine.Type::Item: PurchaseLine.Type:=PurchaseLine.Type::Item;
                SalesLine.Type::Resource: PurchaseLine.Type:=PurchaseLine.Type::Resource; //
                else
                begin
                    IsHandled:=false;
    ...
    begin
        ...
        if lRec_DocumentAttachment.FindSet()then repeat Clear(FileName);
    ...
    begin
        ...
        case true of(SalesHeader.FS_RF_ShipToCode = '') and ShipToNameEqualsSellToName and SalesHeader.FS_RF_ShipToAddressEqualsSellToAddress(): ShipToOptions:=ShipToOptions::"Default (Service provider Address)";
        (SalesHeader.FS_RF_ShipToCode = '') and (not ShipToNameEqualsSellToName or not SalesHeader.FS_RF_ShipToAddressEqualsSellToAddress()): ShipToOptions:=ShipToOptions::"Custom Address";
        SalesHeader.FS_RF_ShipToCode <> '': ShipToOptions:=ShipToOptions::"Alternate Shipping Address";
        end;
    end;
    ...
    begin
        ...
        begin
            ...
            case SalesLine.Type of SalesLine.Type::Item: PurchLine.Validate(Type, PurchLine.Type::Item);
            SalesLine.Type::Resource: PurchLine.Validate(Type, PurchLine.Type::Resource); //
            end;

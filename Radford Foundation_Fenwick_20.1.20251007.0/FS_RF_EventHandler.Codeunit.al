codeunit 50005 FS_RF_EventHandler
{
    ...
    begin
        ...
        if SalesLine.FindSet()then begin
            ...
            repeat TotalFreightPrice+=SalesLine.FS_RF_FreightPrice;
    ...
    begin
        ...
        if DocumentRecord.Number() = Database::"Issued Reminder Header" then if Reminder.GetBySystemId(DocumentRecord.Field(DocumentRecord.SystemIdNo).Value())then begin
                ReminderLines.SetFilter("Reminder No.", Reminder."No.");
                if ReminderLines.FindSet()then repeat if ReminderLines."Document Type" = ReminderLines."Document Type"::Invoice then if SalesInvoice.Get(ReminderLines."Document No.")then begin
        ...
        if TableId = Database::Customer then if DataTypeManagement.FindFieldByName(DocumentRecord, FieldRef, 'Sell-to Customer No.') and Customer.Get(FieldRef.Value())then begin
                SourceTableIDs.Add(Database::Customer);
                SourceIDs.Add(Customer.SystemId);
                SourceRelationTypes.Add(Enum::"Email Relation Type"::"Related Entity".AsInteger());
            end;
        if TableId = Database::Vendor then if DataTypeManagement.FindFieldByName(DocumentRecord, FieldRef, 'Buy-from Vendor No.') and Vendor.Get(FieldRef.Value())then begin
        ...
        if FoundAttachment then begin
            ...
            repeat EmailAddress:=CopyStr(Utilities.GetNextEmailAddressFromCustomReportSelection(CustomReportSelection, DefaultEmailAddress, TempAttachReportSelections.Usage, TempAttachReportSelections.Sequence), 1, MaxStrLen(EmailAddress));
                Clear(TempBlob);
                countFileAttachment:=1;
                ...
                if TableNumber in[Database::"Sales Header", Database::"Sales Invoice Header"]then begin
                    ...
                    if DocumentAttachment.FindSet()then repeat if DocumentAttachment."Document Reference ID".HasValue then begin
                                countFileAttachment+=1;
                                Clear(TempBlob);
    ...
    begin
        RecRef.GetTable(RecordVariant);
        TableNumber:=RecRef.Number;
        if not AllEmailsWereSuccessful then exit;
    ...
    begin
        ...
        if rec.FS_RF_LinkedOrder = '' then exit;
        if rec.FS_RF_SourceType <> Database::"Sales Header" then exit;
    ...
    begin
        ...
        if ContractorShiptoAddress.Get(Rec.FS_RF_ServiceProvider, SalesSetup.FS_RF_DefContrShipToAddr)then Rec.Validate(FS_RF_ContractorShipTo, ContractorShiptoAddress.FS_RF_Code)
        else
        begin
            if not VendorRead then Vendor.Get(Rec.FS_RF_ServiceProvider);
    ...
    begin
        if not PurchInvHeader.IsEmpty then begin
            if PurchInvHeader.FS_RF_LinkedOrder = '' then exit;
    ...
    begin
        if not IsChanged then IsChanged:=SalesInvoiceHeader."External Document No." <> xSalesInvoiceHeader."External Document No.";
    end;
    ...
    var
        SalesLine: Record "Sales Line";
    begin
        case AvailabilityType of 0: begin
            if Item.FS_RF_QtyonBlanketSO = 0 then exit;
    ...
    var
        SalesLine: Record "Sales Line";
    begin
        case ItemAvailabilityLine."Table No." of Database::"Sales Line": begin
        ...
        if not SalesLine.IsCreditDocType()then begin
            if SalesLine."Document Type" = SalesLine."Document Type"::"Blanket Order" then begin
                ...
                if OrderSalesLine.FindSet()then begin
                    repeat OrderQuantity+=OrderSalesLine."Quantity (Base)";

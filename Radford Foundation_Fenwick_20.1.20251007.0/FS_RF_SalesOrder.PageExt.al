pageextension 50015 FS_RF_SalesOrder extends "Sales Order"
{
    layout
    {
        ...
        addafter(ShippingOptions)
        {
            field(FS_RF_ContractorShipTo; Rec.FS_RF_ContractorShipTo)
            {
                ...
                trigger OnValidate()
                begin
                    if Rec.FS_RF_ContractorShipTo <> '' then ShipToOptions:=ShipToOptions::"Custom Address";
                end;
            }
        }
            ...
            group(FS_RF_ServiceDetail)
            {
                ...
                group(FS_RF_Shipping)
                {
                    ...
                    field(FS_RF_ShippingOptions; FS_RF_ShipToOptions)
                    {
                        ...
                        begin
                            case FS_RF_ShipToOptions of FS_RF_ShipToOptions::"Default (Service provider Address)": begin
                                Rec.Validate(FS_RF_ShipToCode, '');
                                Rec.FS_RF_CopySellToAddressToShipToAddress();
                            end;
                            FS_RF_ShipToOptions::"Alternate Shipping Address": begin
                            ...
                            FS_RF_ShipToOptions::"Custom Address": begin
                                Rec.Validate(FS_RF_ShipToCode, '');
        ...
        addafter(CreatePurchaseInvoice)
        {
            action(FS_RF_CreatePurchaseInvoice)
            {
                ...
                begin
                    ...
                    if SalesSetup.Get() and SalesSetup.FS_RF_DisableServiceProvider then PurchDocFromSalesDoc.CreatePurchaseInvoice(Rec, SelectedSalesLine)
                    else
    ...
    begin
        if not(SalesSetup.Get() and SalesSetup.FS_RF_DisableSalesOrderTempl)then begin
            ...
            if SalesOrderTemplates.RunModal() = Action::LookupOK then begin
                ...
                Rec."No.":=NoSeries.GetNextNo(SalesOrderTempl.FS_RF_NoSeries);
            end
            else
            begin
                Error('');
    ...
    protected var FS_RF_ShipToOptions: Option "Default (Service provider Address)", "Alternate Shipping Address", "Custom Address";

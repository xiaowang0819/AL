table 50500 FS_RF_ContractorShipToAddress
{
    ...
    fields
    {
        ...
        field(5; FS_RF_Address; Text[100])
        {
            ...
            trigger OnValidate()
            begin
                FS_RF_PostCodeCheck.VerifyAddress(CurrFieldNo, Database::"Ship-to Address", Rec.GetPosition(), 0, FS_RF_Name, FS_RF_Name2, FS_RF_Contact, FS_RF_Address, FS_RF_Address2, FS_RF_City, FS_RF_PostCode, FS_RF_County, FS_RF_CountryRegionCode);
            end;
        }
        ...
        field(7; FS_RF_City; Text[30])
        {
            ...
            else if(FS_RF_CountryRegionCode=filter(<>''))"Post Code".City where("Country/Region Code"=field(FS_RF_CountryRegionCode));
            ...
            begin
                ...
                PostCodeCheck.VerifyCity(CurrFieldNo, Database::"Ship-to Address", Rec.GetPosition(), 0, FS_RF_Name, FS_RF_Name2, FS_RF_Contact, FS_RF_Address, FS_RF_Address2, FS_RF_City, FS_RF_PostCode, FS_RF_County, FS_RF_CountryRegionCode);
            end;
        }
        ...
        field(15; FS_RF_GLN; Code[13])
        {
            ...
            begin
                if FS_RF_GLN <> '' then GLNCalculator.AssertValidCheckDigit13(FS_RF_GLN);
            end;
        }
        field(16; FS_RF_PostCode; Code[20])
        {
            ...
            else if(FS_RF_CountryRegionCode=filter(<>''))"Post Code" where("Country/Region Code"=field(FS_RF_CountryRegionCode));
            ValidateTableRelation = false;
            ...
            begin
                ...
                PostCodeCheck.VerifyPostCode(CurrFieldNo, Database::"Ship-to Address", Rec.GetPosition(), 0, FS_RF_Name, FS_RF_Name2, FS_RF_Contact, FS_RF_Address, FS_RF_Address2, FS_RF_City, FS_RF_PostCode, FS_RF_County, FS_RF_CountryRegionCode);
            end;
        }
    ...
    procedure Caption(): Text begin
        if FS_RF_VendorNo = '' then exit(UntitledLbl);
        Vendor.Get(FS_RF_VendorNo);
    ...
    begin
        ...
        if not OnlineMapSetup.IsEmpty()then begin
            OnlineMapManagement.MakeSelection(Database::"Ship-to Address", GetPosition())end
        else
        begin
            Message(OnlineMapSetupRequiredLbl);
        end;
    end;
    procedure GetFilterVendorNo(): Code[20]begin
        if GetFilter(FS_RF_VendorNo) <> '' then if GetRangeMin(FS_RF_VendorNo) = GetRangeMax(FS_RF_VendorNo)then exit(GetRangeMax(FS_RF_VendorNo));
    end;
    ...
    begin
        ...
        else if ToAddressID.Get(ToTableNo, ToTableKey, ToAddressType)then ToAddressID.Delete();
    end;
    ...
    begin
        ...
        if FromAddressID.Find('-')then repeat ToAddressID.Init();
                ToAddressID:=FromAddressID;
                ToAddressID."Table No.":=ToTableNo;
                ToAddressID."Table Key":=ToTableKey;
                ToAddressID.Insert();

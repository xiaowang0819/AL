page 50600 FS_RF_ServiceMgrRoleCenter
{
    ...
    layout
    {
        area(rolecenter)
        {
            ...
            part(FS_RF_FinanceMgrRCActivities; FS_RF_FinanceMgrRCActivities)
            {
                ApplicationArea = All;
            }
            part(Control16; FS_RF_ServiceMgrRCActivities)
            {
        ...
        area(Processing)
        {
            ...
            group(Payments)
            {
                ...
                action("Create Vendor Payments")
                {
                    ...
                    RunPageView = where("Document Type"=filter(Invoice), "Remaining Amount"=filter(<0), "Applies-to ID"=filter(''));

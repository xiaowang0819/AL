report 50001 FS_RF_DeliveryDocket
{
    WordLayout = './layouts/DeliveryDocket.docx';
    ...
    dataset
    {
        dataitem(Header; "Sales Header")
        {
            ...
            //Company Info functions were deprecated so added blank variables to not break layout
            column(CompanyLegalOffice; LegalOffice)
            {
            }
            column(CompanyLegalOffice_Lbl; LegalOfficeLbl)
            {
            }
            column(CompanyCustomGiro; CustomGiro)
            {
            }
            ...
            //
            column(VendorAddress1; VendAddr[1])
            {
            }
            column(VendorAddress2; VendAddr[2])
            {
            }
            column(VendorAddress3; VendAddr[3])
            {
            }
            column(VendorAddress4; VendAddr[4])
            {
            }
            column(VendorAddress5; VendAddr[5])
            {
            }
            column(VendorAddress6; VendAddr[6])
            {
            }
            column(VendorAddress7; VendAddr[7])
            {
            }
            column(VendorAddress8; VendAddr[8])
            {
            }
            column(SRVOrder; SRVOrder)
            {
            }
            column(RADDeliveryDate; Format("Requested Delivery Date", 0, 4))
            {
            }
            column(ServiceToAddr1; ServieToAddr[1])
            {
            }
            column(ServiceToAddr2; ServieToAddr[2])
            {
            }
            column(ServiceToAddr3; ServieToAddr[3])
            {
            }
            column(ServiceToAddr4; ServieToAddr[4])
            {
            }
            column(ServiceToAddr5; ServieToAddr[5])
            {
            }
            column(ServiceToAddr6; ServieToAddr[6])
            {
            }
            column(ServiceToAddr7; ServieToAddr[7])
            {
            }
            column(ServiceToAddr8; ServieToAddr[8])
            {
            }
            column(RADPhoneNo; FS_RF_PhoneNo)
            {
            }
            column(RADContact; FS_RF_ShipToContact)
            {
            }
            //
            ...
            column(SellToContactName; ContactName)
            {
            }
            ...
            dataitem(Line; "Sales Line")
            {
                ...
                begin
                    ...
                    Amount:=FS_RF_ContractorCostAmount;
                    "Amount Including VAT":=Amount * (1 + FS_RF_ContractorGSTPercent / 100);
                    TransHeaderAmount+=PrevLineAmount;
                    PrevLineAmount:=FS_RF_ContractorCostAmount;
                    TotalSubTotal+=FS_RF_ContractorCostAmount;
                    TotalAmount+=FS_RF_ContractorCostAmount;
                    TotalAmountVAT+=Amount * FS_RF_ContractorGSTPercent / 100;
                    TotalAmountInclVAT+="Amount Including VAT";
                    OnLineOnAfterGetRecordOnAfterCalcTotals(Header, Line, TotalAmount, TotalAmountVAT, TotalAmountInclVAT);
                    SetPurchaseLine(false, Line.Quantity, Line.FS_RF_ContractorUnitCost, Line."VAT %", Line.FS_RF_ContractorCostAmount, Line."Currency Code", FormattedQuantity, FormattedUnitPrice, FormattedVATPct, FormattedLineAmount);
            ...
            var
                ...
                CustshipToAddress: Record "Ship-to Address";
                Utilities: Codeunit FS_RF_Utilities;
                ArchiveManagement: Codeunit ArchiveManagement;
                SalesPost: Codeunit "Sales-Post";
                ShipToOptions: Option "Default (Service provider Address)", "Alternate Shipping Address", "Custom Address";
            begin
                FirstLineHasBeenOutput:=false;
                Clear(Line);
                Clear(SalesPost);
                ...
                CompanyInfo.Address:=CompanyInfo."Ship-to Address";
                CompanyInfo."Address 2":=CompanyInfo."Ship-to Address 2";
                CompanyInfo.City:=CompanyInfo."Ship-to City";
                CompanyInfo."Post Code":=CompanyInfo."Ship-to Post Code";
                CompanyInfo.County:=CompanyInfo."Ship-to County";
                FormatAddr.GetCompanyAddr("Responsibility Center", RespCenter, CompanyInfo, CompanyAddr);
                Header."Sell-to Contact":='';
                "Bill-to Contact":='';
                FormatAddr.SalesHeaderBillTo(CustAddr, Header);
                Header."Ship-to Contact":=Header."Ship-to Code";
                Header."Ship-to Contact":='';
                ShowShippingAddr:=FormatAddr.SalesHeaderShipTo(ShipToAddr, CustAddr, Header);
                Utilities.CalculateShipToOptions(ShipToOptions, Header);
                if ShipToOptions = ShipToOptions::"Default (Service provider Address)" then begin
                    Clear(VendAddr);
                    if Vendor.Get(FS_RF_ServiceProvider)then FormatAddr.FormatAddr(VendAddr, Vendor.Name, Vendor."Name 2", '', Vendor.Address, Vendor."Address 2", Vendor.City, Vendor."Post Code", Vendor.County, Vendor."Country/Region Code");
                end
                else
                    FormatAddr.FormatAddr(VendAddr, FS_RF_ShipToName, FS_RF_ShipToName2, FS_RF_ShipToCode, FS_RF_ShipToAddress, FS_RF_ShipToAddress2, FS_RF_ShipToCity, FS_RF_ShipToPostCode, FS_RF_ShipToCounty, FS_RF_ShipToCountryRegionCode);
                FormatAddr.SalesHeaderShipTo(VendAddr, CustAddr, Header);
                FormatAddr.FormatAddr(ServieToAddr, FS_RF_ShipToName, FS_RF_ShipToName2, FS_RF_ShipToCode, FS_RF_ShipToAddress, FS_RF_ShipToAddress2, FS_RF_ShipToCity, FS_RF_ShipToPostCode, FS_RF_ShipToCounty, FS_RF_ShipToCountryRegionCode);
                if FS_RF_ShipToCode <> '' then begin
                    if CustshipToAddress.Get("Sell-to Customer No.", FS_RF_ShipToCode)then begin
                        if CustshipToAddress."Phone No." <> FS_RF_PhoneNo then begin
                            FS_RF_PhoneNo:=CustshipToAddress."Phone No.";
                        end;
                    end;
                end;
                if not CompanyBankAccount.Get(Header."Company Bank Account Code")then begin
                    CompanyBankAccount.CopyBankFieldsFromCompanyInfo(CompanyInfo);
                end;
                if not Cust.Get("Bill-to Customer No.")then begin
                    Clear(Cust);
                end;
                ...
                if not IsReportInPreviewMode() and (CurrReport.UseRequestPage and ArchiveDocument or not CurrReport.UseRequestPage and SalesSetup."Archive Orders")then ArchiveManagement.StoreSalesDocument(Header, LogInteraction);
                TotalSubTotal:=0;
    ...
    begin
        if LogInteraction and not IsReportInPreviewMode()then if Header.FindSet()then repeat Header.CalcFields("No. of Archived Versions");
                    if Header."Bill-to Contact No." <> '' then SegManagement.LogDocument(3, Header."No.", Header."Doc. No. Occurrence", Header."No. of Archived Versions", Database::Contact, Header."Bill-to Contact No.", Header."Salesperson Code", Header."Campaign No.", Header."Posting Description", Header."Opportunity No.")
                    else
                        SegManagement.LogDocument(3, Header."No.", Header."Doc. No. Occurrence", Header."No. of Archived Versions", Database::Customer, Header."Bill-to Customer No.", Header."Salesperson Code", Header."Campaign No.", Header."Posting Description", Header."Opportunity No.");
                until Header.Next() = 0;
    end;
    trigger OnPreReport()
    begin
    ...
    ServieToAddr: array[8]of Text[100];
    ...
    local procedure DocumentCaption()DocCaption: Text[250]begin
    ...
    local procedure GetUOMText(UOMCode: Code[10]): Text[50]var
    ...
    local procedure SetPurchaseLine(CommentLine: Boolean; Quantity: Decimal; UnitPrice: Decimal; VATPercentage: Decimal; LineAmount: Decimal; CurrencyCode: Code[10]; var FormattedQty: Text; var FormattedPrice: Text; var FormattedVATPercentage: Text; var FormattedLineAmount: Text)
    var
    ...
    begin
        ...
        begin
            ...
            FormattedPrice:=Format(UnitPrice, 0, AutoFormat.ResolveAutoFormat(AutoFormatType::UnitAmountFormat, CurrencyCode));

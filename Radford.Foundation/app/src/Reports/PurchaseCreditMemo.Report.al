            ...
            dataitem(Line; "Purch. Cr. Memo Line")
            {
                ...
                begin
                    ...
                    if PurchCrMemoLine.Find('-') then begin
                        ...
                        repeat
                            if (PurchCrMemoLine."VAT Identifier" <> VATIdentifier) and (PurchCrMemoLine.Quantity <> 0) then VATAmountText := Text013;
    ...
    begin
        if LogInteraction and not IsReportInPreviewMode() then
            if Header.FindSet() then
                repeat
                    SegManagement.LogDocument(16, Header."No.", 0, 0, Database::Vendor, Header."Buy-from Vendor No.", Header."Purchaser Code", '', Header."Posting Description", '');
                until Header.Next() = 0;
    end;

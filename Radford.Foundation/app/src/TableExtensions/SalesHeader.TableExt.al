tableextension 50011 FS_RF_SalesHeader extends "Sales Header"
{
            ...
            begin
                ...
                if ("Document Type" = "Document Type"::Order) then begin
                    ...
                    if not SalesLine.IsEmpty() then
                        Error(
                          LinkedPurchasesExistLbl,
                          FieldCaption(FS_RF_ContractorShipTo));
                    SalesLine.Reset();
                    ...
                    if CopyShipToAddress then
                        if FS_RF_ContractorShipTo <> '' then begin
    ...
    begin
        ...
        if SalesLine2.FindSet() then begin
            repeat
                TotalFreightPrice += SalesLine2.FS_RF_FreightPrice;
    ...
    procedure FS_RF_HasDifferentShipToAddress(Customer: Record Customer) Result: Boolean
    begin
    ...
    local procedure FS_RF_SellToCustomerIsReplaced(): Boolean
    begin
        exit((xRec."Sell-to Customer No." <> '') and (xRec."Sell-to Customer No." <> "Sell-to Customer No."));
    end;

    local procedure FS_RF_ShipToAddressEqualsOldSellToAddress(): Boolean
    begin
        exit(IsShipToAddressEqualToSellToAddress(xRec, Rec));
    end;

    procedure FS_RF_ShipToAddressEqualsSellToAddress(): Boolean
    begin
    ...
    procedure FS_RF_GetCust(CustNo: Code[20]): Record Customer
    begin
    ...
    begin
        ...
        if ShipToAddr.FS_RF_TaxAreaCode <> '' then
            Validate("Tax Area Code", ShipToAddr.FS_RF_TaxAreaCode);

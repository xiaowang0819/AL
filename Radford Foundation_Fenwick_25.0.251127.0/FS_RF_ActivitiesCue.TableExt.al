tableextension 50016 FS_RF_ActivitiesCue extends "Activities Cue"
{
    fields
    {
        ...
        field(50015; FS_RF_ProjectWaitingRelease; Integer)
        {
        ...
        field(50016; FS_RF_ProjectReleasedOrders; Integer)
        {
        ...
        field(50018; FS_RF_SRVOrderInitialized; Integer)
        {
            ...
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SERV*'), FS_RF_DocumentEmailSent = const(false)));
            FieldClass = FlowField;
        }
        field(50019; FS_RF_SRVOrderSent; Integer)
        {
            Caption = 'SRV Sent Not Released';
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SERV*'), FS_RF_ReadyForShipping = filter(false), FS_RF_DocumentEmailSent = const(true)));
            FieldClass = FlowField;
        }
        field(50020; FS_RF_SRVOrderReleased; Integer)
        {
            Caption = 'SRV Order Released';
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SERV*'), "Completely Shipped" = filter(false), FS_RF_ReadyForShipping = filter(true)));
            FieldClass = FlowField;
        }
        field(50021; FS_RF_SRVOrderContInvoiced; Integer)
        {
            Caption = 'SRV Order Cont Invoiced';
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SERV*'), FS_RF_LinkedPI = filter(<> '')));
            FieldClass = FlowField;
        }
        field(50022; FS_RF_SRVOrderReadyCustInv; Integer)
        {
            Caption = 'SRV Order Ready Cust Inv';
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SERV*'), FS_RF_ApprForCustInvoice = const(true)));
            FieldClass = FlowField;
        }
        field(50023; FS_RF_SRVLinkedPurchInv; Integer)
        {
            Caption = 'SRV Linked Purch Inv';
            CalcFormula = count("Purchase Header" where("Document Type" = filter(Invoice), FS_RF_LinkedOrder = filter(<> '')));
            FieldClass = FlowField;
        }
        field(50024; FS_RF_SRVReleasedOrders; Integer)
        {
            Caption = 'SRV Released Orders';
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SERV*'), "Completely Shipped" = filter(false), FS_RF_ReadyForShipping = filter(true)//Status = filter(Released)
            ));
            FieldClass = FlowField;
        }
        ...
        field(50026; FS_RF_POWaitingRelease; Integer)
        {
            ...
            CalcFormula = count("Purchase Header" where("Document Type" = filter(Order), Status = filter(Open)));
            FieldClass = FlowField;
        }
        field(50027; FS_RF_POReleased; Integer)
        {
            Caption = 'PO Released';
            CalcFormula = count("Purchase Header" where("Document Type" = filter(Order), Receive = const(true), Status = filter(Released)));
            FieldClass = FlowField;
        }
        field(50028; FS_RF_POPendingPrepayment; Integer)
        {
            Caption = 'PO Pending Prepayment';
            CalcFormula = count("Purchase Header" where("Document Type" = filter(Order), Status = filter("Pending Prepayment")));
            FieldClass = FlowField;
        }
        field(50029; FS_RF_POUpcomingOrders; Integer)
        {
            AccessByPermission = tabledata "Purch. Rcpt. Header" = R;
            CalcFormula = count("Purchase Header" where("Document Type" = filter(Order), Status = filter(Released), "Expected Receipt Date" = field(FS_RF_DateFilter)));
        ...
        {
            AccessByPermission = tabledata "Purch. Rcpt. Header" = R;
            CalcFormula = count("Purchase Header" where("Document Type" = filter(Order), "Completely Received" = filter(true), Invoice = filter(false)));
        ...
        {
            AccessByPermission = tabledata "Purch. Rcpt. Header" = R;
            CalcFormula = count("Purchase Header" where("Document Type" = filter(Order), "Completely Received" = filter(true), Invoice = filter(true)));
        ...
        field(50033; FS_RF_SalesOrderReleased; Integer)
        {
            ...
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SO*'), //   Status = filter(Released),
            FS_RF_ReadyForShipping = filter(true), "Completely Shipped" = filter(false)));
            FieldClass = FlowField;
        }
        field(50034; FS_RF_SOWaitingForReleased; Integer)
        {
            ...
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SO*'), //   Status = filter(Released),
            FS_RF_ReadyForShipping = filter(false), "Completely Shipped" = filter(false)));
            FieldClass = FlowField;
        }
        field(50035; FS_RF_ServWaitingForReleased; Integer)
        {
            Caption = 'Service Order Waiting for Released';
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SERV*'), //   Status = filter(Released),
            FS_RF_ReadyForShipping = filter(false), "Completely Shipped" = filter(false)));
            FieldClass = FlowField;
        }
        field(50036; FS_RF_SOPendingPrepayment; Integer)
        {
            ...
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), Status = filter("Pending Prepayment")));
            FieldClass = FlowField;
        }
        field(50037; FS_RF_ServOrderShipped; Integer)
        {
            Caption = 'SRV Order Shipped';
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SERV*'), FS_RF_Shipped = filter(true)));
            FieldClass = FlowField;
        }
        ...
        {
            Caption = 'Sales Order Shipped';
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('SO*'), FS_RF_Shipped = filter(true)));
            FieldClass = FlowField;
        }
        field(50039; FS_RF_ProjReleased; Integer)
        {
            Caption = 'Project Order Released';
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('PROJ*'), //   Status = filter(Released),
            FS_RF_ReadyForShipping = filter(true), "Completely Shipped" = filter(false)));
            FieldClass = FlowField;
        }
        ...
        {
            Caption = 'Project Order Shipped';
            CalcFormula = count("Sales Header" where("Document Type" = filter(Order), "No." = filter('PROJ*'), FS_RF_Shipped = filter(true)));
            FieldClass = FlowField;
        }
    }
